...
/**
 * This special kind of bottom-up tree automaton operates on a tree it is
 * monitoring. It does not only annotate a static tree to determine whether it
 * matches the encoded pattern, but is also able to adapt to changes to the tree
 * without recalculating everything, if the component controlling changes in the
 * tree structure (usually TreeBehavior) announces the changes.
 * 
 * Used by Kahinas breakpoint mechanism as the core device for defining
 * breakpoints. Will dispatch a KahinaTreeMatchEvent to a KahinaController as
 * soon as the encoded pattern is found in the tree it monitors.
 * 
 * @author johannes
 * 
 */
...
public class TreeAutomaton
{
	...
	// link to the breakpoint this automaton implements; contains more
	// contextual info
	KahinaBreakpoint bp;
	...
	/**
	 * set or change the tree this breakpoint is monitoring the automaton will
	 * recompute its annotations in bottom-up manner
	 * 
	 * @param tree
	 *            - the tree model this breakpoint is monitoring
	 */
	 ...
	 * 
	 * @param nodeID
	 *            - the node to be (re)annotated by the automaton
	 */
	public void process(int nodeID)
	{
		// TODO: perhaps introduce a way of detecting the loss of annotations as
		// well
	 ...
	 * 
	 * @param nodeID
	 *            - which node is to be annotated
	 * @param stateID
	 *            - the annotation for the node
	 ...
	 * 
	 * @param nodeID
	 *            - addresses the node whose annotations we want to get
	 ...
	 * 
	 * @param nodeID
	 *            - addresses the node whose children's annotations we want to
	 *            get
	 * @return a set of stateIDs containing all the annotations for the children
	 *         of the node
	 */
	...
	private void announcePatternMatch(int nodeID)
	{
		if (bp.isActive())
		{
			KahinaRunner.processEvent(new KahinaTreeMatchEvent(bp, nodeID));
		}
	}
