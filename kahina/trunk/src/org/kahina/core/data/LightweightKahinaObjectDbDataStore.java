...
public class LightweightKahinaObjectDbDataStore extends DbDataStore
{
    ...
    private static final String TABLE_NAME_PREFIX = LightweightKahinaObjectDbDataStore.class.getSimpleName()
            + "_";
    ...
    private final Class<? extends KahinaObject> type;
    ...
    private void prepareStatements()
    {
        deleteObjectStatement = db.prepareStatement("DELETE FROM "
                ...
                + " AND object_id = ?");
        insertObjectStatement = db.prepareStatement("INSERT INTO "
                + OBJECT_TABLE_NAME
                + " (class_id, object_id, field_id, value_class_id, value_object_id) VALUES ("
                + storeTypeID + ", ?, ?, ?, ?");
        selectObjectStatement = db.prepareStatement("SELECT value_class_id, value_object_id FROM "
                + OBJECT_TABLE_NAME
                + " WHERE class_id = "
    ...
    public KahinaObject retrieve(int id)
    {
        try
        {
            KahinaObject result = constructor.newInstance();
            result.setID(id);
            ...
            for (int i = 0; i < numFields; i++)
            {
                ...
                } else if (fieldType == FieldType.OBJECT_LIST)
                {
                    ResultSet resultSet = selectObjectValue(id, i);
                ...
                } else if (fieldType == FieldType.STRING_LIST)
                {
                    ResultSet resultSet = selectStringValue(id, i);
                ...
                } else if (fieldType == FieldType.INTEGER_LIST)
                {
                    ResultSet resultSet = selectIntegerValue(id, i);
                ...
                } else if (fieldType == FieldType.INT)
                {
                    ...
                    resultSet.next();
                    fields.get(i).set(result, resultSet.getInt(1));
                } else if (fieldType == FieldType.INT_LIST)
                {
                    ResultSet resultSet = selectIntegerValue(id, i);
                    List<Integer> values = new ArrayList<Integer>();
                    while (resultSet.next())
