...
public class KahinaDbChart extends KahinaChart implements LightweightKahinaObject, DatabaseClient
{
    private static final String CLIENT_ID = KahinaDbChart.class.getName();

    private static final String TABLE_NAME_PREFIX = KahinaDbChart.class.getSimpleName()
            + "_";
    ...
    {
        // need no-arg constructor in order to be lightweight
    }
    ...
    {
        int chartID = getID();
        getLeftmostCoveredStatement = db.prepareStatement("SELECT leftmostCovered FROM "
        ...
        setLeftmostCoveredStatement = db.prepareStatement("UPDATE "
                ...
                + chartID);
        getRightmostCoveredStatement = db.prepareStatement("SELECT rightmostCovered FROM "
                + CHART_TABLE_NAME + " WHERE id = " + chartID);
        ...
        getLeftBoundForEdgeStatement = db.prepareStatement("SELECT leftBound FROM " + EDGE_TABLE_NAME
        ...
        insertLeftBoundForEdgeStatement = db.prepareStatement("INSERT INTO "
                ...
                + chartID + ", ?)");
        getRightBoundForEdgeStatement = db.prepareStatement("SELECT rightBound FROM " + EDGE_TABLE_NAME
                + " WHERE id = ? AND chart = " + chartID);
        ...
        insertEdgeCaptionStatement = db.prepareStatement("INSERT INTO "
                ...
                + chartID + ", ?)");
        removeEdgeStatement = db.prepareStatement("DELETE FROM "
                + EDGE_TABLE_NAME + " WHERE id = ? AND chart = " + chartID);
