...
public class KahinaDbChart extends KahinaChart
{
	...
	public void createTablesIfNecessary()
	{
		...
		if (!db.isRegistered(clientID))
		{
			try
			{
				...
				db.execute("CREATE TABLE KahinaCharts " + "(" + "id INT," + "leftBound INT," + "rightBound INT," + "leftmostCovered INT," + "rightmostCovered INT," + "PRIMARY KEY (id)" + ");");
				// create a table for the chart segments
				db.execute("CREATE TABLE KahinaChartSegments" + "(" + "id INT," + "chart INT," + "caption TEXT," + "PRIMARY KEY (id, chart)" + ");");
				// create a table for the chart edges
				db
						.execute("CREATE TABLE KahinaChartEdges" + "(" + "id INT," + "chart INT," + "leftBound INT," + "rightBound INT," + "caption TEXT," + "status INT," + "PRIMARY KEY (id, chart)"
								+ ");");
	...
	{
		int chartID = getID();
		getLeftmostCoveredStatement = db.prepareStatement("SELECT leftmostCovered FROM KahinaCharts WHERE id = " + chartID);
		setLeftmostCoveredStatement = db.prepareStatement("UPDATE KahinaCharts SET leftmostCovered = ? WHERE id = " + chartID);
		getRightmostCoveredStatement = db.prepareStatement("SELECT rightmostCovered FROM KahinaCharts WHERE id = " + chartID);
		setRightmostCoveredStatement = db.prepareStatement("UPDATE KahinaCharts SET rightmostCovered = ? WHERE id = " + chartID);
		getLeftBoundStatement = db.prepareStatement("SELECT leftBound FROM KahinaCharts WHERE id = " + chartID);
		setLeftBoundStatement = db.prepareStatement("UPDATE KahinaCharts SET leftBound = ? WHERE id= " + chartID);
		getRightBoundStatement = db.prepareStatement("SELECT rightBound FROM KahinaCharts WHERE id = " + chartID);
		setRightBoundStatement = db.prepareStatement("UPDATE KahinaCharts SET leftBound= ? WHERE id = " + chartID);
		getSegmentsWithCaptionStatement = db.prepareStatement("SELECT id FROM KahinaChartSegments WHERE chart = " + chartID);
		getSegmentCaptionStatement = db.prepareStatement("SELECT caption FROM KahinaChartSegments WHERE id = ? AND chart = " + chartID);
		updateSegmentCaptionStatement = db.prepareStatement("UPDATE KahinaChartSegments SET caption = ? WHERE id = ? AND chart = " + chartID);
		insertSegmentCaptionStatement = db.prepareStatement("INSERT INTO KahinaChartSegments (id, chart, caption) VALUES (?, " + chartID + ", ?)");
		getEdgeIDsStatement = db.prepareStatement("SELECT id FROM KahinaChartEdges WHERE chart= " + chartID);
		getLeftBoundForEdgeStatement = db.prepareStatement("SELECT leftBound FROM KahinaChartEdges WHERE id = ? AND chart = " + chartID);
		updateLeftBoundForEdgeStatement = db.prepareStatement("UPDATE KahinaChartEdges SET leftBound = ? WHERE edgeID = ? AND chart = " + chartID);
		insertLeftBoundForEdgeStatement = db.prepareStatement("INSERT INTO KahinaChartEdges (id, chart, leftBound) VALUES (?, " + chartID + ", ?)");
		getRightBoundForEdgeStatement = db.prepareStatement("SELECT rightBound FROM KahinaChartEDGES WHERE id = ? AND chart = " + chartID);
		updateRightBoundForEdgeStatement = db.prepareStatement("UPDATE KahinaChartEdges SET rightBound = ? WHERE id = ? AND CHART = " + chartID);
		insertRightBoundForEdgeStatement = db.prepareStatement("INSERT INTO KahinaChartEdges (id, chart, rightBound) VALUES (?, " + chartID + ", ?)");
		getEdgeStatusStatement = db.prepareStatement("SELECT status from KahinaChartEdges WHERE id = ? AND chart = " + chartID);
		updateEdgeStatusStatement = db.prepareStatement("UPDATE KahinaChartEdges SET status = ? WHERE id = ? AND chart = " + chartID);
		insertEdgeStatusStatement = db.prepareStatement("INSERT INTO KahinaChartEdges (id, chart, status) VALUES (?, " + chartID + ", ?)");
		getEdgeCaptionStatement = db.prepareStatement("SELECT caption from KahinaChartEdges WHERE id = ? AND chart = " + chartID);
		updateEdgeCaptionStatement = db.prepareStatement("UPDATE KahinaChartEdges SET caption = ? WHERE id = ? AND chart = " + chartID);
		insertEdgeCaptionStatement = db.prepareStatement("INSERT INTO KahinaChartEdges (id, chart, caption) VALUES (?, " + chartID + ", ?)");
	}
	...
	public Set<Integer> getEdgeIDs()
	{
		try
		{
			return db.queryIntSet(getEdgeIDsStatement);
		} catch (SQLException e)
		{
			throw new KahinaException("SQL error.", e);
		}
